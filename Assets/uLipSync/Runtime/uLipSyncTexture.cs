...
public class uLipSyncTexture : MonoBehaviour
{
    ...
    void Apply()
    {
        ...
        // Ensure we are working with the instance copy (not shared)
        var mats = targetRenderer.materials;

        // Check if the 4th material exists
        if (mats.Length < 4) return;

        if (_initialTexture == null)
        {
            _initialTexture = mats[3].mainTexture;
        }
        ...
        foreach (var item in textures)
        {
            ...
            var texture = item.texture ?? _initialTexture;
            if (texture && texture != mats[3].mainTexture)
            {
                mats[3].mainTexture = texture;
            }

            mats[3].mainTextureScale = item.uvScale;
            mats[3].mainTextureOffset = item.uvOffset;
        }
